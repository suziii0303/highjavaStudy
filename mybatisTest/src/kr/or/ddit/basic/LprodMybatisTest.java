...
public class LprodMybatisTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 1-2. 환경 설정 파일을 읽어와 환경 설정을 완성한 후 SQL문을 호출해서 실행할 수 있는 객체들
		...
		try {
			...
			//		'논리값'이 true면 AutoCommit이 활성화된 상태이고 
			//		'논리값'이 생략되거나 false이면 AutoCommit이 비활성화된 상태이다.
			session = sqlSessionFactory.openSession();
			
			// SqlSession객체를 이용하여 처리할 SQL 문을 호출하여 실행한다.
...
//			int insertCnt = session.insert("lprod.insertLprod1",lvo1);
		...
		*/
		...
		System.out.println("삭제할 lorodGu 입력 >>");
		...
		*/
		...
		try {
			...
			// 응답 결과가 여러개일 경우 lelectList()메서드를 사용하는데 이 메서드는
			// 여러개의 레코드 각각을 VO객체에 담은 후 이 VO데이터를 List에 추가해 주는
			// 작업을 자동으로 수행한다.
		...
		*/
		...
		try {
			...
			LprodVO lvo5 = session.selectOne("lprod.selectOne",lprodGu4);
