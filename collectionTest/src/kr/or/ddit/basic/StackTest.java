...
// 웹브라우저의 앞으로 가기, 뒤로 가기 기능 구현(Stack아용)
class Browser{
	...
	// 뒤로가기
	public void goBack() {
		//isEmpty()메서드 ==> 컬렉션이 비어 있으면 true, 그렇지 않으면 false를 반환한다
	...
	public void goForward() {
		if(!forward.isEmpty()) {
			back.push(currentURL);		// 현재 페이지를 back 스택에 추가한다.
			currentURL = forward.pop(); // forward 스택에 1개의 요소를 꺼내와 현재페이지로 설정한다.
		}
	}
