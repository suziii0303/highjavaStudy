...
//		System.out.println("Set데이터 => "+ hs1);
		...
		/*
		Set의 데이터는 순서(index)가 없기 때문이 List처럼 인덱스로 데이터를 하나씩 불러올 수 없다.
		...
		// Iterator객제의 hasNext()메서드
		//		==> Iterator의 데이터를 가리키는 포인터가 현재 가리키는 위치의 다음 번째에 데이터가 잇는지 검사한다.
